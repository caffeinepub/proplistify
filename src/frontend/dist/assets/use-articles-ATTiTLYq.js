...
const SAMPLE_ARTICLES = [
  {
    ...
    metaDescription: "Discover the most promising real estate investment neighborhoods of 2025, with expert analysis on price trends, ROI projections, and growth potential.",
    content: `
...
Investing wisely requires thorough research. Our team of experts is ready to help you identify the right opportunity.
    ...
    publishedAt: (Date.now() - 7 * 24 * 3600 * 1e3) * 1e6,
    updatedAt: (Date.now() - 5 * 24 * 3600 * 1e3) * 1e6,
  ...
  },
  {
    ...
    metaDescription: "Everything you need to know as a first-time home buyer: financing, inspections, negotiations, and closing — explained step by step.",
    content: `
...
Take your time, do your research, and don't let anyone pressure you into a decision you're not comfortable with.
    `.trim(),
    featuredImage: "/assets/generated/blog-hero.jpg",
    publishedAt: (Date.now() - 14 * 24 * 3600 * 1e3) * 1e6,
    updatedAt: (Date.now() - 12 * 24 * 3600 * 1e3) * 1e6,
  ...
  },
  {
    ...
    metaDescription: "Explore how shifting lifestyle preferences and remote work are transforming the luxury rental market across major cities worldwide.",
    content: `
...
The luxury rental market is expected to grow 15% annually through 2028, making it one of the most attractive segments for investors.
    `.trim(),
    featuredImage: "/assets/generated/blog-hero.jpg",
    publishedAt: (Date.now() - 21 * 24 * 3600 * 1e3) * 1e6,
    updatedAt: (Date.now() - 20 * 24 * 3600 * 1e3) * 1e6,
    author: "Priya Sharma",
    isPublished: true
  }
];
function useArticles(page = 1, limit = 9) {
  return useQuery({
...
function useRecentArticles(count = 3) {
  return useQuery({
    queryKey: ["recent-articles", count],
    queryFn: async () => SAMPLE_ARTICLES.filter((a) => a.isPublished).sort((a, b) => b.publishedAt - a.publishedAt).slice(0, count),
    staleTime: 6e4
  });
}
...
export {
  useArticles as a,
  useArticleBySlug as b,
  useAllArticles as c,
  useDeleteArticle as d,
  useUpdateArticle as e,
  useArticle as f,
  useCreateArticle as g,
  useRecentArticles as u
};
